#!/bin/bash
# Task: Configure DNS record for Open WebUI
# Phase: 5 (Wiki + LLM Platform — Sub-phase B)
# Number: 10
# Prerequisites:
#   - Phase 2 complete (Pi-hole running)
#   - Configuration loaded (SERVER_IP, OPENWEBUI_DOMAIN)
# Parameters:
...
#   Phase 2 (task-ph2-08) established this pattern.
...
DNS_RECORD="${SERVER_IP} ${OPENWEBUI_DOMAIN}"
...
# Check idempotency
if echo "$CURRENT_JSON" | grep -q "${OPENWEBUI_DOMAIN}"; then
    print_success "Open WebUI DNS record already exists in Pi-hole dns.hosts — skipping"
    # Verify DNS actually resolves
    if nslookup "${OPENWEBUI_DOMAIN}" 127.0.0.1 &> /dev/null; then
        print_success "DNS resolution verified: ${OPENWEBUI_DOMAIN} -> ${SERVER_IP}"
    else
        print_info "DNS record exists but not resolving — restarting Pi-hole..."
        docker restart pihole
        sleep 8
        if nslookup "${OPENWEBUI_DOMAIN}" 127.0.0.1 &> /dev/null; then
            print_success "DNS resolution verified after restart: ${OPENWEBUI_DOMAIN} -> ${SERVER_IP}"
        else
...
if [[ "$DRY_RUN" == true ]]; then
    print_info "[DRY-RUN] Would add Open WebUI DNS record to Pi-hole dns.hosts"
    print_info "[DRY-RUN] Record: ${DNS_RECORD}"
else
    print_info "Adding Open WebUI DNS record to Pi-hole dns.hosts..."
    ...
    if docker exec pihole pihole-FTL --config dns.hosts "${NEW_HOSTS_JSON}"; then
        print_success "Open WebUI DNS record added to Pi-hole"
    else
        print_error "Failed to update Pi-hole dns.hosts"
        exit 1
    fi

    # Full container restart required for FTL to pick up new records
    ...
    if nslookup "${OPENWEBUI_DOMAIN}" 127.0.0.1 &> /dev/null; then
        RESOLVED_IP=$(nslookup "${OPENWEBUI_DOMAIN}" 127.0.0.1 | grep -A1 "Name:" | grep "Address:" | awk '{print $2}')
    ...
    print_success "Open WebUI DNS record configured"
